...
| `idle` | Idle-accrual base: an Assignment component + Engine + ready TickSystem that drives developer-supplied Activities (`engine/idle.Producer`) and routes their Output into economy/progression/inventory. Buildings/lifeskills/stages are plugins, not core. | `HookOutput`, `HookState` |
| `loot` | Weighted-drop mechanism: a concurrency-safe `Roller` over a game-supplied `EntriesFor` lookup (DB, content pack, hardcoded map). No persistence, no item catalog. | `HookRoll` |
| `quest` | Pure objective-tracking: `Advance(steps, currentStep, progress, actionType, target, amount)` decides step/quest completion with no I/O. A game owns persistence and rewards. | — |
| `vitals` | Live HP/MP pool registry: concurrency-safe deduct/heal/spend-MP/revive, with a race-safe deduct-and-reserve-kill primitive (exactly one credited kill under concurrent attackers). | — |
