...
func TestEndToEndGameLoop(t *testing.T) {
	...
	creatures := makeCreatures(charService)
	combatService := combat.NewCombatService(creatures, statClient, sessionReg, killreward.New(creatures, charService, questService, lootService, invService, nil), nil, nil, nil)
	...
	mux.HandleFunc("/ws", socket.ServeWS(context.Background(), hub, jwtSecret, wsMsgHandler, wsDisconnectHandler))
...
func TestDoubleSessionOverride(t *testing.T) {
	...
	mux.HandleFunc("POST /api/v1/auth/login", auth.LoginHandler(authService))
	mux.Handle("POST /api/v1/characters", auth.AuthMiddleware(jwtSecret)(character.CreateHandler(charService)))
	mux.HandleFunc("/ws", socket.ServeWS(context.Background(), hub, jwtSecret, wsMsgHandler, nil))
...
func TestDeadAttackerAndDefender(t *testing.T) {
	...
	sessionReg := session.NewRegistry()
	creatures2 := makeCreatures(charService)
	combatService := combat.NewCombatService(creatures2, statClient, sessionReg, killreward.New(creatures2, charService, questService, lootService, invService, nil), nil, nil, nil)
...
func TestInvalidJWTToken(t *testing.T) {
	...
	hub := socket.NewHub()
	go hub.Run()
	mux.HandleFunc("/ws", socket.ServeWS(context.Background(), hub, "my-secret", nil, nil))
